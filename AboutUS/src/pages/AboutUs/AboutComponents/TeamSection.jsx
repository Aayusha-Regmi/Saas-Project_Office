import React from 'react';

const TeamSection = () => {
  const teamMembers = [
    {
      id: 1,
      image: "/images/img_group_1171276020.png",
      socialIcon: "/images/img_group_1171276047.svg"
    },
    {
      id: 2,
      image: "/images/img_group_1171276020.png",
      socialIcon: "/images/img_group_1171276047.svg"
    },
    {
      id: 3,
      image: "/images/img_group_1171276020.png",
      socialIcon: "/images/img_group_1171276047.svg"
    },
    {
      id: 4,
      image: "/images/img_group_1171276020.png",
      socialIcon: "/images/img_group_1171276047.svg"
    }
  ];

  return (
    <section className="py-16">
      <div className="container mx-auto px-4">
        <div className="flex items-center justify-center mb-10">
          <div className="bg-[#22a8ff] w-14 h-14 rounded-full flex items-center justify-center mr-4">
            <span className="text-3xl font-semibold text-white">%</span>
          </div>
          <h2 className="text-3xl font-semibold text-[#32b5fd]">Our Team</h2>
        </div>
        
        <h3 className="text-4xl font-bold text-[#071e45] text-center mb-12">
          Meet With Our Experts
        </h3>
        
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {teamMembers.map(member => (
            <div key={member.id} className="relative group">
              <img 
                src={member.image} 
                alt="Team Member" className="w-full h-auto"
              />
              <div className="absolute bottom-0 right-0">
                <img 
                  src={member.socialIcon} 
                  alt="Social Media" className="w-10 h-10 cursor-pointer transition-transform hover:scale-110"
                />
              </div>
            </div>
          ))}
        </div>
      </div>
    </section>
  );
};

export default TeamSection;